...
const {
  normalizeCropName,
} = require('../utils/crop-normalizer');

const {
  getRecommendations:
    getMember12Recommendations,
...
function createRecommendationOrchestrator({
  ...
  async function getRecommendations(request) {
    ...
    if (
      Array.isArray(member12Response)
    ) {
    ...
    for (
      const item
      of member3Response.marketScores
    ) {
